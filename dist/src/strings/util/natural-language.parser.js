...
class NaturalLanguageParser {
    static parse(query) {
        ...
        if (longer) {
            parsed.min_length = Number(longer[1]) + 1;
            matched = true;
        }
        ...
        if (shorter) {
            ...
            if (Number.isFinite(n) && n > 0) {
                parsed.max_length = n - 1;
                matched = true;
            }
        }
        const contains = q.match(/\bcontain(?:ing)?\s+(?:the\s+letter\s+)?([a-z])\b/) ||
            q.match(/\bhas\s+letter\s+([a-z])\b/);
        if (contains) {
            parsed.contains_character = contains[1];
            matched = true;
        }
